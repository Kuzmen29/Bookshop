...
export default function StarRating({ comment, setComment }: { comment: IUserComment, setComment: (x: any) => void }) {
        ...
        <div className={styles['star-rating']}>
            {
                stars.map((item, index) => (
                    <svg width="32px"
                        ...
                        onMouseOver={() => {
                            !comment.rating && setStars(() => [0, 0, 0, 0, 0].map((item, i) => i <= index ? 1 : 0))
                        }}
                        onMouseLeave={() => !comment.rating && setStars([0, 0, 0, 0, 0])}
                        onClick={() => {
                            if (!comment.rating) {
                                setComment({ ...comment, rating: index + 1 })
                            } else {
